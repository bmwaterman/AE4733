function [receiver_position,epsilon_c]  = Trilateration_Matlab_Function_Clockbias( satellite_positions, measured_ranges )
...
while (some_condition)
    ...
    some_condition = ~ (norm(measured_ranges - [norm(receiver_position_guess_n - satellite_positions(:, 1));...
        norm(receiver_position_guess_n - satellite_positions(:, 2));...
        norm(receiver_position_guess_n - satellite_positions(:, 3));...
        norm(receiver_position_guess_n - satellite_positions(:, 4))])) < epsilon_close && ~ (n_iter >= max_iterations);
